...
inline void prog() {
    ...
    for(int i=st; i<n-1; i++)
        if(arr[i] > arr[i + 1]) e = i + 1;
    // bool f = st == 0 || arr[e] > arr[st - 1];
    // bool b = e == n - 1 || arr[st] < arr[e + 1];
    vi tmp;
    for(int i=0; i<st; i++) tmp.push_back(arr[i]);
    for(int i=e; i>=st; i--) tmp.push_back(arr[i]);
    for(int i=e+1; i<n; i++) tmp.push_back(arr[i]);
