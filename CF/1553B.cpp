...
inline void prog() {
    ...
    for(int i=0; i<(int)s.length(); i++) {
        if(s[i] == t.front()) {
            for(int rcnt=0; rcnt<(int)t.length(); rcnt++) {
                int rln = 1 + rcnt;
                if(rln > (int)t.length()) break;
                if(rcnt + i >= (int)s.length()) break;
                int lln = (int)t.length() - rln;
                if(lln > rcnt + i) continue;
                string tmp;
                int cnt=0;
                for(int k=i; k<=rcnt + i; k++) tmp += s[k];
                for(int k=rcnt + i - 1; cnt++ < lln; k--) tmp += s[k];
                if(tmp == t)  {
                    cout<<"YES\n";
                    return;
                }
