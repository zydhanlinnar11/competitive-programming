...
inline void dfs(vi adjList[], int &n, int &m, int mx, int curr, int node, vi &color, int &ans, vi &cat) {
    color[node] = 1;
    mx = max(mx, curr);
    bool isLeaf = true;
    ...
    for(int &i: adjList[node]) {
        if(color[i] == 0) {
            dfs(adjList, n, m, mx, (cat[i] ? curr + 1 : 0), i, color, ans, cat);
        }
    }
    color[node] = 2;
}
...
inline void prog() {
    int n, m, ans = 0;
    cin>>n>>m;
    vi adjList[n], cats(n), color(n, 0);
    for(int &i: cats) cin>>i;
    for(int i=0; i<n-1; i++) {
        int u, v;
        cin>>u>>v;
        u--, v--;
    ...
    dfs(adjList, n, m, 0, cats[0], 0, color, ans, cats);
    cout<<ans<<"\n";
}
