...
inline void prog() {
    ...
    int x_cnt = count(arr.begin(), arr.end(), mx);
    ll ans = fct[n];
    int y_cnt = count(arr.begin(), arr.end(), mx - 1);
    int z_cnt = n - y_cnt - x_cnt;
    for(int i=0; i<=z_cnt && x_cnt == 1; i++) {
        ans -= modmul(fct[n - i - x_cnt], modmul(comb(z_cnt, i, fct), perm(i, i, fct)));
        ans %= MOD;
    }
    ans += MOD;
    ans %= MOD;
